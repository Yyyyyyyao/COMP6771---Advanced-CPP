	...
	public:
		...
		public:
			...
			auto operator++() -> iterator& {
				// if the inner iterator is not the end
				if (set_iter_ != map_iter_->second.end()) {
					++set_iter_;
					// if the inner iterator is not the end after we move to the next
					// return iterator
					if (set_iter_ != map_iter_->second.end()) {
						return *this;
					}
				}
				// the inner iterator hit the end()
				// move the outer iterator to the next
				std::cout << pointee_->size() << std::endl;
				++map_iter_;
				// std::cout << std::distance(map_iter_, pointee_->end()) << std::endl;
				// if the outer iterator hit the end,
				// I assign empty to inner iterator;
				// else assign the begin of the new outer iterator
				// set_iter_ = map_iter_ == pointee_->end() ? set_iterator() :
				// map_iter_->second.begin();
				set_iter_ = map_iter_ == map_iter_end_ ? set_iterator() : map_iter_->second.begin();
				return *this;
			}
			...
			auto operator--() -> iterator& {
				// if the outer iterator is at the end()
				// move it to the last element in the outer container
				// move the inner iterator to the last element in the inner container at the current
				// outer container and return
				...
				// if the inner iterator is at the begin()
				if (set_iter_ == map_iter_->second.begin()) {
					// if the outer iterator is at the begin() as well
					// set inner iterator to empty because no more elements ahead
					// if (map_iter_ == pointee_->begin()) {
					// 	set_iter_ = set_iterator();
					// }
					// else {
					// if the outer iterator is not at the begin()
					// move outer iterator to the prev one
					// set inner iterator to the end of the inner container
					--map_iter_;
					set_iter_ = map_iter_->second.end();
					// }
				}
			...
			auto operator==(iterator const& other) const -> bool {
				if (other.map_iter_ == pointee_->end() && map_iter_ == pointee_->end()) {
					std::cout << "yes1 " << pointee_->size() << std::endl;
					return true;
				}
				if (other.map_iter_ != map_iter_) {
					return false;
				}
				std::cout << "yes3 " << pointee_->size() << std::endl;
				return other.set_iter_ == set_iter_;
			};

		private:
			// pointee is used to find the end() and begin() of the map
		...
		graph(graph&& other) noexcept {
			...
			//                  std::map<std::shared_ptr<N>, std::set<edge, edge_cmp>,
			//                  edge_map_cmp>{});
		...
		auto merge_replace_node(N const& old_data, N const& new_data) -> void {
			...
			// Check if the old_data node have edges
			// if not, delete the old_data node in nodes_ set directly
			if (found_iter != edges_.end()) {
				// if old_data node has edges
				for (auto map_iter = edges_.begin(); map_iter != edges_.end(); ++map_iter) {
					if (map_iter->second.empty()) {
						continue;
					}
					// three cases
					// 1. for an edge, src is the old data
					// 2. for an edge, dst is the old data
					// 3. for an edge, both src and dst are old data
				...
				// after looping through all edges,
				// I need to erase the map element with key = old_data
		...
		[[nodiscard]] auto is_connected(N const& src, N const& dst) const -> bool {
			...
			// src, dst pair for searching in edge set of a src
