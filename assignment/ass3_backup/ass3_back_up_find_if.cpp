...
namespace gdwg {
	...
	public:
		...
		auto insert_edge(N const& src, N const& dst, E const& weight) -> bool {
			...
			// auto iter = std::find_if(edges_.begin(), edges_.end(), [&src, &dst, &weight](auto const&
			// it) { 	return *it.from == src && *it.to == dst && *it.weight == weight;
			// });
			if (iter == edges_.end()) {
				auto another_src = std::find_if(nodes_.begin(), nodes_.end(), [&src](auto const& it) {
					return *it == src;
				});
				auto another_dst = std::find_if(nodes_.begin(), nodes_.end(), [&dst](auto const& it) {
					return *it == dst;
				});
				auto new_edge = edge{};
				new_edge.from = *another_src;
				new_edge.to = *another_dst;
				auto another_weight = std::find_if(weights_.begin(),
				                                   weights_.end(),
				                                   [&weight](auto const& it) { return *it == weight; });
		...
		auto merge_replace_node(N const& old_data, N const& new_data) -> void {
			...
			auto old_data_it = std::find_if(nodes_.begin(), nodes_.end(), [&old_data](auto const& it) {
				return *it == old_data;
			});
		...
		auto erase_node(N const& value) -> bool {
			auto old_data_it = std::find_if(nodes_.begin(), nodes_.end(), [&value](auto const& it) {
				return *it == value;
			});
		...
		auto erase_edge(N const& src, N const& dst, E const& weight) -> bool {
			...
			auto iter = std::find_if(edges_.begin(), edges_.end(), [&src, &dst, &weight](auto const& it) {
				return *it.from == src && *it.to == dst && *it.weight == weight;
			});
			if (iter == edges_.end()) {
				return false;
			}
		...
		[[nodiscard]] auto is_node(N const& value) const -> bool {
			...
			auto iter = std::find_if(nodes_.begin(), nodes_.end(), [&value](auto const& it) {
				return *it == value;
			});
		...
		[[nodiscard]] auto is_connected(N const& src, N const& dst) const -> bool {
			...
			auto find_edge_iter = std::find_if(edges_.begin(), edges_.end(), [&src, &dst](auto const it) {
				return *it.from == src && *it.to == dst;
			});
		...
		[[nodiscard]] auto find(N const& src, N const& dst, E const& weight) const -> iterator {
			auto find_edge_iter =
			   std::find_if(edges_.begin(), edges_.end(), [&src, &dst, &weight](auto const it) {
				   return *it.from == src && *it.to == dst && *it.weight == weight;
			   });
